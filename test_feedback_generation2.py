...
try:
    ...
    if len(questions_list) != NUM_QUESTIONS:
        print(f"경고: 로드된 질문 개수({len(questions_list)})가 설정된 질문 개수({NUM_QUESTIONS})와 다릅니다. 설정값을 확인해주세요.")
...
def generate_with_gemini(model_name: str, prompt: str, temperature: float = 0.7, max_retries=3) -> str:
...
def generate_user_answer_gemini(question: str, level: int) -> str:
    ...
    user_answer = generate_with_gemini(MODEL_NAME_FOR_ANSWER, prompt, temperature=temperature)
    return user_answer
...
피드백:"""
    feedback = generate_with_gemini(MODEL_NAME_FOR_FEEDBACK, prompt, temperature=0.5) # temperature 낮춰서 요약 집중
    return feedback
...
def main():
    ...
    for question in questions_list:
        for level in range(1, NUM_ANSWERS_PER_QUESTION + 1):
            ...
            if not user_answer:
                print(f"  -> 수준 {level} 사용자 답변 생성 실패. 다음 시도로 넘어갑니다.")
            ...
            if generated_count % SAVE_BATCH_SIZE == 0:
                output_filepath = pathlib.Path(OUTPUT_DIR) / f"feedback_data_batch_{batch_number}.json"
                try:
