...
OUTPUT_JSON_FILEPATH = (
    "/Users/sondain/careerbee/cs_feedback_data.json"  # 저장할 JSON 파일 경로
)
...
=======
        print(
            f"경고: 로드된 질문 개수({len(questions_list) * NUM_ANSWERS_PER_QUESTION})가 목표 샘플 개수({TARGET_SAMPLE_COUNT})보다 적습니다."
        )
        TARGET_SAMPLE_COUNT = (
            len(questions_list) * NUM_ANSWERS_PER_QUESTION
        )  # 목표 개수를 질문 개수로 조정
>>>>>>> 9e08925 (API 키 보안 수정)
...
=======
        ...
        generated_data.append(
            {"질문": question, "사용자 답변": user_answer, "피드백": feedback}
        )
        generated_count += 1  # 성공적으로 생성된 경우 카운트 증가
